...
- ThreadSocketHandler : Sends/Receives data from peers on port 9988.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9986 for connections and services them.
...
However, in open source software such as Pprfnk Core, where everyone uses
...
pay attention to for reviewers of Pprfnk Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Pprfnk Core uses tinyformat, which is type safe. Leave them out to avoid confusion
