...
const char * const BITCOIN_CONF_FILENAME = "pprfnk.conf";
const char * const BITCOIN_PID_FILENAME = "pprfnkd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pprfnk" is a composite category enabling all Pprfnk-related debug output
            if(ptrCategory->count(string("pprfnk"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PprfnkCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PprfnkCore
    // Mac: ~/Library/Application Support/PprfnkCore
    // Unix: ~/.pprfnkcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PprfnkCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PprfnkCore";
#else
    // Unix
    return pathRet / ".pprfnkcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pprfnk.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pprfnk.conf
